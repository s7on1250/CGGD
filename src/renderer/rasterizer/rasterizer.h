...
namespace cg::renderer
{
	...
	public:
		...
		//For edge mode
		void set_edge_mode(bool edge_mode);
				...
				for (float y = bounding_box_begin.y; y<= bounding_box_end.y;y+=1.f){
					...
					if(edge0 >= 0.f && edge1 >= 0.f && edge2 >= 0.f)
					{
						...
						if(depth_test(depth, u_x,u_y)){
							...
							if(edge_mode&&(std::abs(edge0)<=200||std::abs(edge1)<=200||std::abs(edge2)<=200)){
								auto line_color = cg::unsigned_color::from_float3({1,1,0});
								render_target->item(u_x,u_y) = line_color;
							}
							else
							{
	...
	template<typename VB, typename RT>
	void rasterizer<VB, RT>::set_edge_mode(bool e)
	{
		this->edge_mode = e;
	}
